...
public class Jorge extends RobotHandler {
    ...
    public void newDrive() {
        ...
        float strafe = opMode.gamepad1.right_stick_x;
        float rotate = opMode.gamepad1.left_stick_x;
        float slow = opMode.gamepad1.left_trigger;
        ...
        drive.setPowers(new String[]{"rf", "rb", "lf", "lb"}, new double[]{powerRF, powerRB, powerLF, powerLB});
        doBeaconPresser(opMode.gamepad1.left_bumper);

        Gamepad gamepad2 = opMode.gamepad2;
        doPickUp(opMode.gamepad1.right_bumper, opMode.gamepad1.b);
        load(gamepad2.right_trigger, gamepad2.x);
        doShooting((opMode.gamepad1.right_trigger > .25));
        doFixShooter(gamepad2.a);
    ...
    public void newDrive2() {
        ...
        //Gamepad gamepad2 = opMode.gamepad2;
        doPickUp(opMode.gamepad1.right_bumper, opMode.gamepad1.b);
        load(opMode.gamepad1.left_stick_button, opMode.gamepad1.right_stick_button);
        doShooting((opMode.gamepad1.right_trigger > .25));
        doFixShooter(opMode.gamepad1.a);
    ...
    protected void doFixShooter (boolean trigger) {
        if (shooting) return;
        if (!fixing && trigger && !shooting) { fixing = true; }
        ...
        if (fixing) {
            if (!set) {
                ...
                //shooter.setPower(.2);
                //servos.setPosition(LOADER, LOAD);
                auxMotors.setTargetPosition(SHOOTER, (SHOOTER_FIRE/2), .5);
                set = true;
            }
            if (/*(shooter.getCurrentPosition() > SHOOTER_FIRE || shooter.getCurrentPosition() == SHOOTER_FIRE)*/
                    (auxMotors.getPosition(SHOOTER) > (SHOOTER_FIRE/2) || auxMotors.getPosition(SHOOTER) == (SHOOTER_FIRE/2)) && set) {
                //shooter.setPower(0);
                //shooter.setTargetPosition(0);
                //shooter.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                //shooter.setMode(DcMotor.RunMode.RUN_TO_POSITION);
    ...
    protected void load (boolean override, boolean button) {
        if (override) {
            if (button) {
                servos.setPosition(LOADER, UNLOAD);
            } else {
    ...
    protected void doPickUp (boolean toggleButton, boolean backwards) {
        if (backwards) {
            auxMotors.setPower(PICKUP, .4); return;
        }
        if (!pickupToggled && toggleButton) {
            if (!pickupOn) {
                //pickUp.setPower(RUN_PICK_UP);
